...
Provides API interfaces for managing prices within <%= application_name %>.

## Get All Prices

```shell
curl "<%= curl_url %>/api/prices" \
...
```shell
curl -XPOST "<%= curl_url %>/api/prices" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
```shell
curl -XPUT "<%= curl_url %>/api/prices/1" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{
        "price": {
...
```shell
curl -XDELETE "<%= curl_url %>/api/prices/1/deactivate" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`PUT <%= api_url %>/api/prices/:id/deactivate`
