...
```shell
curl -XPOST "<%= curl_url %>/api/instances/:instanceId/deploys"
  -H "Authorization: BEARER <%= curl_token %>"
...
```shell
curl -XPUT "<%= curl_url %>/api/deploys/:id"
  -H "Authorization: BEARER <%= curl_token %>"
  -H "Content-Type: application/json" \
  -d '{"appDeploy":{
    "config": { }
...
```shell
curl -XPOST "<%= curl_url %>/api/deploys/:id/deploy"
  -H "Authorization: BEARER <%= curl_token %>"
  -H "Content-Type: application/json" \
  -d '{"appDeploy":{
    "config": { }
...
```shell
curl -XDELETE "<%= curl_url %>/api/deploys/:id" \
  -H "Authorization: BEARER <%= curl_token %>"
```
