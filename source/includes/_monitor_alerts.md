...
```shell
curl "<%= curl_url %>/api/monitoring/alerts"
...
`GET <%= api_url %>/api/monitoring/alerts`
...
```shell
curl "<%= curl_url %>/api/monitoring/alerts/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/monitoring/alerts/:id`
...
```shell
curl -XPOST "<%= curl_url %>/api/monitoring/alerts" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
`POST <%= api_url %>/api/monitoring/alerts`
...
```shell
curl -XPUT "<%= curl_url %>/api/monitoring/alerts/1" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '"alert": {
    "minSeverity": "critical",
...
`PUT <%= api_url %>/api/monitoring/alerts/:id`
...
```shell
curl -XDELETE "<%= curl_url %>/api/monitoring/alerts/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/monitoring/alerts/:id`
