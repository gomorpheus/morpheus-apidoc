...
```shell
curl "<%= curl_url %>/api/networks/proxies"
...
`GET <%= api_url %>/api/networks/proxies`
...
```shell
curl "<%= curl_url %>/api/networks/proxies/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/networks/proxies/:id`
...
```shell
curl -XPOST "<%= curl_url %>/api/networks/proxies" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
`POST <%= api_url %>/api/networks/proxies`
...
```shell
curl -XPUT "<%= curl_url %>/api/networks/proxies/:id" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{
  "networkProxy": {
...
`PUT <%= api_url %>/api/networks/proxies/:id`
...
```shell
curl -XDELETE "<%= curl_url %>/api/networks/proxies/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/networks/proxies/:id`
