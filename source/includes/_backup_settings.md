...
Provides API interfaces for managing backup settings within <%= application_name %>.

## Get Backup Settings

```shell
curl "<%= curl_url %>/api/backup-settings" \
...
`GET <%= api_url %>/api/backup-settings`


## Update Backup Settings

```shell
curl -XPUT "<%= curl_url %>/api/backup-settings" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
`PUT <%= api_url %>/api/backup-settings`
...
backupAppliance | N | When enabled, a Backup will be created to backup the <%= application_name %> appliance database
