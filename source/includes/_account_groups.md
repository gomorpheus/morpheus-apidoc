...
```shell
curl -XPOST "<%= curl_url %>/api/accounts/20/groups" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
```shell
curl -XPUT "<%= curl_url %>/api/accounts/20/groups/365" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{"group":{
    "name": "My Group",
...
```shell
curl -XPUT "<%= curl_url %>/api/accounts/20/groups/365/update-zones" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{"group":{
    "zones": [
...
```shell
curl -XDELETE "<%= curl_url %>/api/accounts/20/groups/365" \
  -H "Authorization: BEARER <%= curl_token %>"
```
