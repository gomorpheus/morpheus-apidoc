...
```shell
curl "<%= curl_url %>/api/processes/250" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
```shell
curl "<%= curl_url %>/api/processes/events/940" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/processes/events/:id`
