...
Provides API interfaces for managing provisioning settings within <%= application_name %>.

## Get Provisioning Settings

```shell
curl "<%= curl_url %>/api/provisioning-settings" \
...
`GET <%= api_url %>/api/provisioning-settings`


## Update Provisioning Settings

```shell
curl -XPUT "<%= curl_url %>/api/provisioning-settings" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
`PUT <%= api_url %>/api/provisioning-settings`
