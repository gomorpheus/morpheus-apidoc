...
```shell
curl -XPOST "<%= curl_url %>/api/projects" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
```shell
curl -XPUT "<%= curl_url %>/api/projects/:id" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{"project":{
    "addInstances":[
...
```shell
curl -XDELETE "<%= curl_url %>/api/projects/:id" \
  -H "Authorization: BEARER <%= curl_token %>"
```
