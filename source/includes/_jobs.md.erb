...
Provides API interfaces for managing jobs within <%= application_name %>.

## Get All Jobs

```shell
curl "<%= curl_url %>/api/jobs" \
...
```shell
curl -XPUT "<%= curl_url %>/api/jobs/3/execute" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`PUT <%= api_url %>/api/jobs/:id/execute?customConfig=%7Bfoo%3Abar%7D`
...
```shell
curl -XDELETE "<%= curl_url %>/api/jobs/3" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
```shell
curl "<%= curl_url %>/api/job-executions" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/job-executions`
...
```shell
curl "<%= curl_url %>/api/job-executions/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/job-executions/:id`
...
```shell
curl "<%= curl_url %>/api/job-executions/1/events/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/job-executions/:id/events/:eventId`
