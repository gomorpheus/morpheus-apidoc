...
```shell
curl "<%= curl_url %>/api/policy-types" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
```shell
curl -XPOST "<%= curl_url %>/api/policies" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
```shell
curl -XPUT "<%= curl_url %>/api/policies/1" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{
  "policy": {
...
```shell
curl -XDELETE "<%= curl_url %>/api/policies/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
```shell
curl "<%= curl_url %>/api/groups/1/policies"
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/groups/:groupId/policies`
...
```shell
curl "<%= curl_url %>/api/zones/1/policies"
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/zones/:cloudId/policies`
