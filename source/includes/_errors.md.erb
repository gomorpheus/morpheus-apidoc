...
When the <%= application_name %> API encounters an error, the response will have an HTTP status of **400** of greater, instead of **200 OK**.  The error response body contains JSON with information to help troubleshoot the error.

### 400 Bad Request

```shell
curl -XPOST "<%= curl_url %>/api/contacts" \
...
The <%= application_name %> API uses the following error codes:
