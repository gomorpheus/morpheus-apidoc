...
```shell
curl "<%= curl_url %>/api/environments"
...
```shell
curl "<%= curl_url %>/api/environments/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/environments/:id`
...
```shell
curl -XPOST "<%= curl_url %>/api/environments" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
```shell
curl -XPUT "<%= curl_url %>/api/environments/5" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{
  "environment": {
...
`PUT <%= api_url %>/api/environments/:id`
...
```shell
curl -XPUT "<%= curl_url %>/api/environments/5/toggle-active" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`PUT <%= api_url %>/api/environments/:id/toggle-active`
...
```shell
curl -XDELETE "<%= curl_url %>/api/environments/5" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/environments/:id`
