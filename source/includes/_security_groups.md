...
A Security Group is a grouping of rules.  Each rule is a whitelist entry for a particular IP address to either a port range or a particular <%= application_name %> instance type.  A Security Group may be applied to multiple Clouds, Apps, and Instances.

## Get All Security Groups

```shell
curl "<%= curl_url %>/api/security-groups"
...
`GET <%= api_url %>/api/security-groups`
...
```shell
curl "<%= curl_url %>/api/security-groups/18" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/security-groups/:id`
...
```shell
curl -XPOST "<%= curl_url %>/api/security-groups" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
`POST <%= api_url %>/api/security-groups`
...
```shell
curl -XPUT "<%= curl_url %>/api/security-groups/18" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{ "securityGroup": {
  "name": "My New Security Group",
...
`PUT <%= api_url %>/api/security-groups/:id`
...
```shell
curl -XDELETE "<%= curl_url %>/api/security-groups/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/security-groups/:id`
...
```shell
curl -XPOST "<%= curl_url %>/api/security-groups/18/locations" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
...
`POST <%= api_url %>/api/security-groups/:id/locations`
...
```shell
curl -XDELETE "<%= curl_url %>/api/security-groups/18/locations/29" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/security-groups/:id/locations/:locationId`
