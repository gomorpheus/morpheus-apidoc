...
```shell
curl -XPOST "<%= curl_url %>/api/budgets" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
```shell
curl -XPUT "<%= curl_url %>/api/budgets/:id" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{"budget":{
    "interval": "quarter",
...
```shell
curl -XDELETE "<%= curl_url %>/api/budgets/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
