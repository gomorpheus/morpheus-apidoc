...
```shell
curl "<%= curl_url %>/api/catalog/types" \
...
```shell
curl "<%= curl_url %>/api/catalog/types/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/catalog/types/:id`
...
```shell
curl "<%= curl_url %>/api/catalog/items" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
```shell
curl "<%= curl_url %>/api/catalog/items/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/catalog/items/:id`
...
```shell
curl "<%= curl_url %>/api/catalog/cart" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/catalog/cart`

## Add Catalog Item to Cart

```shell
curl -XPOST "<%= curl_url %>/api/catalog/cart/items" \
  -H "Authorization: Bearer <%= curl_token %>" \
...
`POST <%= api_url %>/api/catalog/cart/items`
...
```shell
curl -XDELETE "<%= curl_url %>/api/catalog/cart/items/:id" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/catalog/cart/items/:id`
...
```shell
curl -XDELETE "<%= curl_url %>/api/catalog/cart" \
  -H "Authorization: Bearer <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/catalog/cart`

## Checkout Catalog Cart

```shell
curl -XPOST "<%= curl_url %>/api/catalog/checkout" \
  -H "Authorization: Bearer <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{
}'
...
`POST <%= api_url %>/api/catalog/checkout`


## Place Catalog Order

```shell
curl -XPOST "<%= curl_url %>/api/catalog/orders" \
  -H "Authorization: Bearer <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{
  "order": {
...
`POST <%= api_url %>/api/catalog/orders`
...
```shell
curl -XDELETE "<%= curl_url %>/api/catalog/items/:id" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/catalog/items/:id`
