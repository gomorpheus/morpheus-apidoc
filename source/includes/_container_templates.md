...
```shell
curl "<%= curl_url %>/api/library/container-templates" \
...
```shell
curl "<%= curl_url %>/api/library/container-templates/27" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
```shell
curl -XPOST "<%= curl_url %>/api/library/container-templates" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
```shell
curl -XPUT "<%= curl_url %>/api/library/container-templates/:id" \
  -H "Authorization: BEARER <%= curl_token %>"
  -H "Content-Type: application/json" \
  -d '{"containerTemplate": {
    "name":"appconfig.yml"
...
```shell
curl -XDELETE "<%= curl_url %>/api/library/container-templates/:id" \
  -H "Authorization: BEARER <%= curl_token %>"
```
