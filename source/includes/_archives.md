...
```shell
curl "<%= curl_url %>/api/archives/buckets" \
...
`GET <%= api_url %>/api/archives/buckets`
...
```shell
curl "<%= curl_url %>/api/archives/buckets/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/archives/buckets/:id`
...
```shell
curl -XPOST "<%= curl_url %>/api/archives/buckets" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
`POST <%= api_url %>/api/archives/buckets`
...
```shell
curl -XPUT "<%= curl_url %>/api/archives/buckets/1" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
  -d '{
  "archiveBucket": {
...
`PUT <%= api_url %>/api/archives/buckets/1`
...
```shell
curl -XDELETE "<%= curl_url %>/api/archives/buckets/1" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/archives/buckets/:id`
...
```shell
curl "<%= curl_url %>/api/archives/buckets/mybucket/files/" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/archives/buckets/:bucket/files/:filePath`
...
```shell
curl "<%= curl_url %>/api/archives/files/954" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/archives/files/:fileId`
...
```shell
curl -XPOST "<%= curl_url %>/api/archives/buckets/mybucket/files/myapp/config/?filename=application.rb" \
  -H "Authorization: BEARER <%= curl_token %>" \
...
`POST <%= api_url %>/api/archives/buckets/:bucket/files/:filePath`
...
```shell
curl -XGET "<%= curl_url %>/api/archives/download/mybucket/myapp/config/application.rb" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/archives/download/:bucket/:filePath`
...
```shell
curl -XGET "<%= curl_url %>/public-archives/download/mybucket/pubdemo/GREETINGS.md"
```
...
`GET <%= api_url %>/public-archives/download/:bucket/:filePath`
...
```shell
curl -XDELETE "<%= curl_url %>/api/archives/files/99" \
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`DELETE <%= api_url %>/api/archives/files/:fileId`
...
```shell
curl "<%= curl_url %>/api/archives/files/1/links"
  -H "Authorization: BEARER <%= curl_token %>"
```
...
`GET <%= api_url %>/api/archives/files/:fileId/links`
...
```shell
curl -XPOST "<%= curl_url %>/api/archives/files/:fileId/links" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
```
...
```shell
curl -XDELETE "<%= curl_url %>/api/archives/files/1/links/1" \
  -H "Authorization: BEARER <%= curl_token %>" \
  -H "Content-Type: application/json" \
```
...
`DELETE <%= api_url %>/api/archives/files/:fileId/links/:linkId`
...
```shell
curl -XGET "<%= curl_url %>/public-archives/link?s=45a214fce9a546b9"
```
...
`GET <%= api_url %>/public-archives/link?s=:secretAccessToken`
